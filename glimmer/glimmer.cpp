	...
	for( int i = 0; i < N; i++ ) {		
		for( int j = 0; j < n_original_dims; j++ ) {
			if( (max_vals[j] - min_vals[j]) < 0.0001f ) {
				...
				if( !isfinite( data[i*(n_original_dims)+j]) ) 
					data[i*(n_original_dims)+j] = 0.f;
			}
		}
	}
...
void force_directed( int size, int fixedsize, float* distmat ) {
	...
	for( int i = fixedsize; i < size; i++ ) {
		...
		for( int j = 0; j < V_SET_SIZE+S_SET_SIZE; j++ ) {
			...
			g_idx[i*(V_SET_SIZE+S_SET_SIZE)+j].highd=(float) distmat[i*N-akk+idx];
...
{
	N = 6;
	double* distmat_test = new double[15];
	/*
	distmat[0] = 1.0;
	distmat[1] = 3.0;
	distmat[2] = 5.0;
	...
	*/
	...
	//		}
		...
		while( !g_done ) {
			...
			// move the points
			if( g_interpolating )
				force_directed( g_heir[ g_current_level ], g_heir[ g_current_level+1 ] , distmat);
			else
